...
export const authOptions = {
  ...
  callbacks: {
    ...
    async jwt({ token, user, trigger, session }) {
      // ─── Case 1: Initial sign-in — user object is only present on first call ───
      if (user?.id) {
        await dbConnect();
      ...
      // ─── Case 2: Session update trigger ───────────────────────────────────────
      if (trigger === "update" && session?.user?.userType) {
        token.userType = session.user.userType;
      }

      // ─── Case 3: Access token still valid — return as-is, NO DB call ─────────
      if (Date.now() < Number(token.accessTokenExpires || 0)) {
        return token;
      }

      // ─── Case 4: Access token expired — verify refresh token & rotate ─────────
